...
class Test {
    public static void main(String[] args) {
        ...
        System.err.println(Arrays.toString(search(arr, 6)));
    ...
    static int[] search(int[][] matrix, int target) {
        ...
        int cols = matrix[0].length; // be cautious matrix may contain empty
        ...
        if (matrix[rStart][cMid] == target) {
            return new int[] { rStart, cMid }; // start
        }
        if (matrix[rStart + 1][cMid] == target) {
            return new int[] { rStart + 1, cMid }; // end
        }
